...
//     const response = await this.fetchFromTMDB<TMDBResponse<TVShow>>(`/tv/${tvId}/recommendations`)
//     return response.results
//   }
...
class TMDBApi {
  ...
  async getTVSeasonDetails(tvId: number, seasonNumber: number): Promise<any> {
    return await this.fetchFromTMDB<any>(`/tv/${tvId}/season/${seasonNumber}`)
  }

  async getTVEpisodeDetails(tvId: number, seasonNumber: number, episodeNumber: number): Promise<any> {
    return await this.fetchFromTMDB<any>(`/tv/${tvId}/season/${seasonNumber}/episode/${episodeNumber}`)
  }
}
